    ...
    <div class="margin-top-10 tabhost">
        <div class="tabhost-title">
            <ul>
                ...
                <li class="active"><a href="<?php echo Yii::app()->createUrl('systems/transmit/create'); ?>"><?php
                        if ($action == 'create') {
                            echo '新建文章';
                        } else {
                            echo '修改文章';
                ...
                <div class="row add_article_list">
                    <div class="left">
                        ...
                        if (!empty($model->time_start)) {
                            $time_start = date('Y-m-d', $model->time_start);
                        } else {
                            $time_start = date('Y-m-d', time());
                        }
                        if (!empty($model->time_end)) {
                            $time_end = date('Y-m-d', $model->time_end);
                        } else {
    ...
    "link","unlink","highlightcode","|","undo","redo","source"]],
                ...
                <div class="row add_article_list">
                    ...
                    <div class="left">
                        ...
                        <?php echo $form->dropDownList($model, 'status', Yii::app()->params->TRANSMITSTATUS); ?>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        function childtype(i) {
            var url = "<?php echo Yii::app()->createUrl('systems/transmit/ChildtransmitType'); ?>";
            var id = i;//需替换为动态获取
            ...
            function (data) {
                ...
                $('#Transmit_child_type_id').html(html);
            }, 'json');
        }
        $('#Transmit_type_id').change(function () {
            childtype($(this).val());
            forwarding();
        });
        $('#Transmit_type_id').val(<?php echo @$model->transmit_type->parent_id; ?>);
        $('#Transmit_type_id').change();
        setTimeout(function () {
            $('#Transmit_child_type_id').val(<?php echo @$model->type_id; ?>);
        }, 1000);
        //forwarding();
        function forwarding() {
