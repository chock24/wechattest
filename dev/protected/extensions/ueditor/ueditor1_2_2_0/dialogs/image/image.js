    ...
    function insertSearch(id, catchRemote) {
        ...
        for (var i = 0, ci; ci = imgs[i++]; ) {
            if (ci.getAttribute("selected")) {
                ...
                var use_url = ci.getAttribute("data_ue_src", 2).replace(/(\s*$)/g, ""), img = {};
                img.src = use_url;
                img.data_ue_src = url;
                imgObjs.push(img);
            }
                    ...
                    if (!list.children.length) {
                        ajax.request(editor.options.imageManagerUrl, {
                            ...
                            onsuccess: function(xhr) {
                                ...
                                var resultArr = !xhr.responseText ? [] : xhr.responseText.split("ue_separate_ue");//结果数组
                                var imageArr = [];
                                for (var k = 0, ci; ci = resultArr[k++]; ) {
                                    var imageSource = ci.split("self_separate_self");
                                    imageArr[k] = [];
                                    imageArr[k].push(imageSource);
                                }
                                ...
                                for (var i = 0; i < imageArr.length; i++) {
                                    if (imageArr[i]) {
                                        ...
                                        img.setAttribute(i < 35 ? "src" : "lazy_src", editor.options.imageManagerPath + imageArr[i][0][0]);
                                        img.setAttribute("data_ue_src", editor.options.imageManagerPath + imageArr[i][0][1]);
                                    }
                                }
//                                g( "imageList" ).innerHTML = !length ? "&nbsp;&nbsp;当前未上传过任何图片！" : "";
